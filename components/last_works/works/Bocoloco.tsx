...
interface BocolocoProps {
    isHover: boolean;
}

const Bocoloco: React.FC<BocolocoProps> = ({ isHover }) => {
            ...
            <div className="key-infos">
                ...
                {!isHover ? (
                    <div className="isHovered">
                        ...
                        <ul>
                            {traduction[lang].lastWork.bocoloco.projects.one.list.map((list: string, i: number) => {
                                return <li key={i}>{list}</li>;
                            })}
                        </ul>
                        <h4>{traduction[lang].lastWork.bocoloco.projects.two.title}</h4>
                        <ul>
                            {traduction[lang].lastWork.bocoloco.projects.two.list.map((list: string, i: number) => {
                                return <li key={i}>{list}</li>;
                            })}
                        </ul>
                    </div>
                    ...
                    <>
                        ...
                        <div className="languages">
                            ...
                            {traduction[lang].lastWork.bocoloco.languages.map((icon: string, i: number) => {
                                return <Logos key={i} type={icon as any} size="3em" />;
                            })}
                        </div>
                        <div className="devtools">
                            <h4>{traduction[lang].lastWork.bocoloco.devToolsTitle}</h4>
                            {traduction[lang].lastWork.bocoloco.contentDevtools.map((tools: string, i: number) => (
                                <p key={i}>{tools}</p>
                            ))}
                        </div>
                    </>
